...
def cmd_list(
    ...
    from .display import _s, path_info, pattern_entry
    ...
    for section_label, _config_key, entries in sections:
        ...
        if search:
            ...
            filtered = [
                e
                for e in filtered
                if term in e.get("description", "").lower() or term in e.get("pattern", "").lower()
            ]
...
def cmd_init(
    ...
    starter patterns. Test patterns are DISABLED by default — the user
    must review and enable them. Use --with-examples to also copy all
    files from the bundled ``examples/`` directory (enabled by default).

    Args:
        with_examples: Copy all bundled example files into the config.
    ...
    # Always write the test config (disabled)
    test_config = _build_minimal_starter_config()
    _write_init_yaml(test_config, config_path / "00-test.yaml")
    saved_files = ["00-test.yaml"]

    total_block = len(test_config.get("patterns", []))
    total_allow = len(test_config.get("allow_patterns", []))
    total_deny = len(test_config.get("deny_patterns", []))

    if with_examples:
        # Copy all bundled example files (enabled by default — no `enabled: false`)
        example_files = _copy_example_files(config_path)
        for fname, b, a, d in example_files:
    ...
    lines = [
        success(
            f"Created: [path]{config_path}/[/path] [dim]({', '.join(sorted(saved_files))})[/dim]"
        ),
        f"  [dim]{total_block} block, {total_allow} allow, {total_deny} deny patterns[/dim]",
        "",
        "[bold]Next steps:[/bold]",
        "  1. Review the config: [bold]hermes custom-dangerous-patterns list[/bold]",
        "  2. Enable test patterns:"
        " [bold]hermes custom-dangerous-patterns"
        " enable --group testing[/bold]",
...
def _copy_example_files(config_path: Path) -> list[tuple[str, int, int, int]]:
    """Copy all bundled example YAML files into the config directory.

    Discovers ``*.yaml`` files in the plugin's ``examples/`` directory
    (excluding ``00-test.yaml``) and copies each as-is into
    *config_path*. Patterns are written with their original enabled
    state — the example files are enabled by default.

    Returns a list of ``(filename, block_count, allow_count, deny_count)``
    tuples for the init output message.
    """
    from .config import _clean_for_serialization, _load_single_yaml, _validate_config, _write_yaml

    # Try the shipped examples directory first, then the installed plugin path
    candidates = [
        Path(__file__).resolve().parent / "examples",
        Path.home() / ".hermes" / "plugins" / "custom-dangerous-patterns" / "examples",
    ]

    examples_dir = next((p for p in candidates if p.is_dir()), None)
    ...
    for yaml_file in sorted(examples_dir.glob("*.yaml")):
        if yaml_file.name == "00-test.yaml":
            continue  # Handled separately by cmd_init

        raw = _load_single_yaml(yaml_file)
        if not raw:
            continue
        ...
        # Write the file (preserving original enabled state)
        output: dict[str, Any] = {}
        for key in ("patterns", "allow_patterns", "deny_patterns"):
        ...
        if output:
            target_path = config_path / yaml_file.name
            _write_yaml(output, target_path)

            block_count = len(config.get("patterns", []))
            allow_count = len(config.get("allow_patterns", []))
            deny_count = len(config.get("deny_patterns", []))
            saved.append((yaml_file.name, block_count, allow_count, deny_count))
...
def cmd_enable(
    ...
    """
    return _toggle_patterns(
        enable=True, target=target, pattern_type=pattern_type, group=group, dry_run=dry_run
    )
...
def cmd_disable(
    ...
    """
    return _toggle_patterns(
        enable=False, target=target, pattern_type=pattern_type, group=group, dry_run=dry_run
    )
...
def _toggle_patterns(
    ...
    if group:
        ...
        if not matched:
            matched = [e for e in all_entries if target.lower() in e.get("description", "").lower()]
            if pattern_type:
            ...
            if len(matched) > 1:
                lines = [
                    f"[warning]⚠[/warning] Multiple patterns match "
                    f"'[bold]{target}[/bold]'. Use index or be more "
                    f"specific:"
                ]
    ...
    if already and not dry_run:
        names = ", ".join(
            f'[{get_index(e, all_entries)}] [bold]"{e["description"]}"[/bold]' for e in already
        )
...
def cmd_validate(
    ...
    from .display import check, cross, path_info, success, warn_mark
    ...
    if raw is None:
        ...
        lines.append("")
        lines.append(
            "[bold red]Result: INVALID[/bold red]"
            " — fix the YAML syntax above"
...
def cmd_info() -> tuple[str, int]:
    ...
    from .display import check, cross, muted, path_info, subheading, warn_mark
    ...
    if not config_path.exists():
        ...
        lines.append(
            muted("  Run `hermes custom-dangerous-patterns init` to create a starter config.")
        )
        return ("\n".join(lines) + "\n", 0)
    ...
    for label, key in count_sections:
        ...
        if active and disabled:
            lines.append(
                f"  [bold]{label}:[/bold] {len(entries):>3}"
                f" [dim]({active} active,"
                f" {disabled} disabled)[/dim]"
            )
        elif active:
            lines.append(f"  [bold]{label}:[/bold] {len(entries):>3} [dim]({active} active)[/dim]")
        elif entries:
            lines.append(
                f"  [bold]{label}:[/bold] {len(entries):>3} [dim]({disabled} disabled)[/dim]"
            )
        else:
...
def cmd_add(
    ...
    return _add_noninteractive(
        ...
        group or "",
        examples or [],
        enabled_flag if enabled_flag is not None else True,
        protected,
        dry_run,
...
def _add_interactive(
    ...
    # --- Glob entry ---
    try:
        glob_input = input("Enter a glob-style pattern (or press Enter for raw regex): ").strip()
    except (EOFError, KeyboardInterrupt):
        return ("\nCancelled.\n", 1)
    ...
    example: str = ""
    try:
        ex_input = input("\nEnter an example command to test (press Enter to skip): ").strip()
    except (EOFError, KeyboardInterrupt):
        return ("\nCancelled.\n", 1)
                ...
                try:
                    ...
                    if glob_input:
                        ...
                        try:
                            ...
                            print(f"  Error: generated regex is invalid: {compile_exc}")
                        else:
                            print("Re-testing...")
                            if compiled.search(example):
                                print(f"  \u2713 {example}")
...
def _remove_interactive(
    ...
    from .config import remove_entry_from_file, save_config
    from .display import _console
    ...
    # In directory/combined mode, disable the entry (write to 99-custom.yaml)
    # instead of deleting from source files — source files are user-managed.
    if not config_path.is_dir():
...
def _remove_by_target(
    ...
    if not matched:
        ...
        if len(matched) > 1:
            lines = [
                f"[warning]⚠[/warning] Multiple patterns match"
                f" '[bold]{target}[/bold]'."
                f" Use index or be more specific:"
            ]
            for e in matched:
                idx = get_index(e, all_entries)
                type_label = e["_section"].replace("_patterns", "").upper()
    ...
    if entry.get("protected"):
        return (
            f"[error]✗[/error] Pattern [{eidx}]"
            f' [bold]"{entry["description"]}"[/bold]'
            f" is protected.\n"
            f"Edit the config file directly to"
            f" remove protected patterns.\n",
            1,
        )

    if dry_run:
        return (
            f"[bold]Would remove {type_label}"
            f" pattern [{eidx}]:[/bold]"
            f' [bold]"{entry["description"]}"[/bold]\n'
            f"Use without --dry-run to confirm.\n",
            0,
    ...
    if not force:
        ...
        render_and_print("")
        render_and_print(
            f"[bold]Matched pattern:[/bold] [{eidx}] [bold]{type_label}:[/bold] {desc_safe}"
        )
        try:
            confirm = input("Remove this pattern? This cannot be undone. [y/N]: ").strip().lower()
        except (EOFError, KeyboardInterrupt):
    ...
    return (
        f"[success]✓[/success] Removed {type_label}"
        f" pattern [{eidx}]:"
        f' [bold]"{entry["description"]}"[/bold]\n'
...
# Built-in patterns come from Hermes at runtime via tools.approval_detection.
# We do NOT bundle a static copy: that drifted from Hermes's actual list,
# bloated plugin source, and tripped the plugin security scanner on every
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    list_p.add_argument("--disabled", action="store_true", help="Show only disabled patterns")
    list_p.add_argument("--enabled", action="store_true", help="Show only enabled patterns")
    list_p.add_argument(
        "--builtins",
        action="store_true",
    ...
    # -- test --
    test_p = subs.add_parser("test", help="Test a command against all pattern types")
    ...
    # -- init --
    init_p = subs.add_parser("init", help="Create a starter config file")
    init_p.add_argument(
        "--with-examples",
        action="store_true",
    ...
    # -- validate --
    validate_p = subs.add_parser("validate", help="Validate config syntax and regexes")
    ...
    # -- logs --
    logs_p = subs.add_parser("logs", help="Show plugin log entries")
    logs_p.add_argument(
        "-l",
        "--level",
